...
/* ================================================================
   FIREBASE CONFIG
   Loaded from js/env.js — which is in .gitignore (safe).
   Keys never appear in source code committed to GitHub.

   How it works:
     1. index.html loads js/env.js first (sets window.__ENV__)
     2. _getEnv() reads from window.__ENV__
     3. FIREBASE_CONFIG picks up all values automatically

   To update keys:
     → Edit js/env.js only — never edit this file directly
   ================================================================ */

/**
 * _getEnv(key, fallback?)
 * Read a value from window.__ENV__ (set by js/env.js).
 * Returns fallback if key is missing.
 *
 * @param {string} key
...
export const FEATURES = Object.freeze({
  FIREBASE_ENABLED:      true,    // ✅ ENABLED — env.js se keys load ho rahi hain
